...
if uploaded_files:
    ...
    for i, file in enumerate(uploaded_files, 1):
        ...
        if "error" not in data:
            ...
            # ─── DEBUG AVANCÉ ───
            with st.expander("Debug détaillé (pour analyser les cas difficiles)"):
                st.markdown(f"**Durée** : {data['duration']:.1f} secondes")
                st.markdown(f"**Tuning offset** : {data['tuning']:+.2f} cents")
                st.markdown(f"**Segments valides** : {data['valid_seg']}")
                st.markdown(f"**Segments analysés (même invalides)** : {len(data['segment_timeline'])}")
                
                if data.get("modulation"):
                    st.warning(f"Modulation tardive détectée vers → {data['modulation']} ({CAMELOT_MAP.get(data['modulation'], '??')})")
